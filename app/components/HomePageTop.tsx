...
export default function MobileTopBar() {
  ...
  const launcherBtnRef = useRef<HTMLButtonElement>(null); // return focus on close
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`${API_BASE_URL}/api/show-logo/?_=${Date.now()}`, {
          ...withFrontendKey(),
          signal: ac.signal,
        });
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setError(null);
      try {
        const url = `${API_BASE_URL}/api/show_nav_items/?_=${Date.now()}`;
        const res = await fetch(url, { ...withFrontendKey(), signal: ac.signal });
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        const json: CategoryRaw[] = await res.json();
        if (!cancelled) setNavData(Array.isArray(json) ? json : []);
      } catch (err: unknown) {
        if (!cancelled) setError((err as Error)?.message || "Failed to load items");
      } finally {
      ...
      if (u) {
        ...
        if (!displayName) {
          try {
            ...
            if (snap.exists()) displayName = (snap.data() as any).username || null;
          } catch {
            /* ignore */
          }
        }
  ...
  const syncAfterModalClose = useCallback(async () => {
    ...
    try {
      ...
      if (found) {
        const niceName =
          found.name || found.username || found.first_name || (found.email ? String(found.email).split("@")[0] : "User");
  ...
  const onChipClick = useCallback((text: string) => {
    setSearchQuery(text);
    setShowSearch(true);
