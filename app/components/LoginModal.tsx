...
// Redirect target after Firebase-hosted verification page
...
// ---- Helper: normalize Firebase error messages for UX
...
const LoginModal: React.FC<LoginModalProps> = ({
  ...
  // Debounced username check (backend)
  useEffect(() => {
    if (!isSignup) {
      setNameUnique(null);
  ...
  // Debounced email check during SIGNUP (backend + Firebase)
  useEffect(() => {
    if (!isSignup) {
      setEmailUnique(null);
    ...
    emailDebounceTimer.current = window.setTimeout(async () => {
      try {
        ...
        try {
          ...
          // ignore; we still have backend signal
        }
  ...
  const canSubmit = useMemo(() => {
    if (isSignup) {
      ...
      // 🔒 Require policy acceptance to enable "SIGN UP"
  ...
  // Backend saves (after Firebase success). NO password field.
  const saveUserToBackend = async ({
    ...
    if (!response.ok) {
      ...
      // Allow conflict-like cases to pass quietly if it's just uniqueness
      if (String(msg).toLowerCase().includes("unique")) return;
      throw new Error(msg);
    }
  };
  ...
  // Google sign-in (block unverified, just in case)
  const handleGoogleLogin = async () => {
    try {
      ...
      // Some providers can come unverified; enforce verification hard-stop
      await reload(result.user);
      if (!result.user.emailVerified) {
        try {
          await sendEmailVerification(result.user, { url: VERIFICATION_CONTINUE_URL });
        } catch {}
        await signOut(auth); // ✅ block session for unverified
  ...
  const handleAuth = async () => {
    ...
    try {
      ...
      if (isSignup) {
        ...
        // ✅ MUST succeed in Firebase. No backend-only fallback.
        ...
        try {
          ...
          return; // Wait until verified; we persist after verification
        } catch (e) {
        ...
        // SIGN IN — Firebase only, block unverified sessions
        ...
        try {
          ...
          // Force refresh and block if unverified
          await reload(result.user);
          if (!result.user.emailVerified) {
            try {
              await sendEmailVerification(result.user, { url: VERIFICATION_CONTINUE_URL });
            } catch {}
            await signOut(auth); // ✅ hard block
          ...
          // Verified → ensure backend has record (no password sent)
          try {
            ...
            // If edit fails (e.g., no row), attempt save
            try {
              await saveUserToBackend({
                user_id: result.user.uid,
                email,
        ...
        >
          <h2 className="text-[22px] md:text-[26px] font-semibold mb-4 md:mb-5">
            {isSignup ? (
              <>
                ...
                <img src={ICONS.user} alt="user icon" className="inline-block w-5 h-5 align-middle" />
              </>
            ) : (
              <>
                Sign In{" "}
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={ICONS.lock} alt="lock icon" className="inline-block w-7 h-7 align-middle" />
              </>
            )}
          </h2>
          ...
          <div className="relative w-full">
            ...
            <input
              ...
              aria-invalid={
                isSignup
                  ? (identifier.length > 0 && !isValidEmail(identifier)) || emailUnique === false
                  : false
              }
              value={identifier}
                ...
                >
                  <svg
                    ...
                    className={`h-3 w-3 transition-opacity duration-150 ${
                      acceptedPolicy ? "opacity-100" : "opacity-0"
                    }`}
                  >
          ...
          {awaitingVerification && (
            <div className="mt-4 w-full text-left text-sm border rounded-lg p-3 bg-[#891F1A] text-white border-amber-200">
              <div className="font-semibold mb-1">Verify your email</div>
              <div className="mb-2">
                We sent a verification link to <span className="font-medium">{verifEmail || identifier}</span>. Open it, then click “I’ve verified” below.
              </div>
