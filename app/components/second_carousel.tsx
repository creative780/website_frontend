...
export default function SecondCarousel() {
    ...
    (async () => {
      try {
        ...
        const [navRes, carRes] = await Promise.all([
          ...
          fetch(`${baseUrl}/api/second-carousel/?_=${Date.now()}`, withFrontendKey({ signal })),
        ]);
        ...
        if (carRes.ok) {
          ...
          setCarouselRaw((p) => ({ ...p, images: [] }));
        }
      } catch (err: any) {
        if (err?.name === 'AbortError') return;
        console.error('❌ second-carousel fetch error:', err);
  ...
  const displayImages = useMemo(() => {
    return carouselRaw.images.map((img) => {
      ...
      return { src: img.src, title: img.title, caption: img.caption, href };
    });
  ...
  const scrollRight = useCallback(() => setCurrentIndex((prev) => Math.min(maxIndex, prev + 1)), [maxIndex]);
  ...
  const carouselId = 'second-carousel';
  ...
  return (
    ...
    >
      <header className="text-center w-3/4 m-0">
        <h2 id={`${carouselId}-title`} className="text-[#891F1A] text-2xl sm:text-3xl font-semibold text-center mb-2">
          {carouselRaw.title}
        </h2>
                  ...
                  {(item.title || item.caption) && (
                    <div className="py-2 flex justify-between items-start gap-2">
                      <div className="flex-1 text-center">
                        {item.title && <h3 className="text-sm font-medium text-[#333]">{item.title}</h3>}
                        {item.caption && <p className="text-xs font-normal text-[#666]">{item.caption}</p>}
                      </div>
                    </div>
                  )}
                </Link>
            ...
            <div className="w-full flex items-center justify-center">
              <div className="grid grid-cols-4 gap-[10px] w-full">
                {Array.from({ length: 4 }).map((_, i) => (
                  <div key={i} className="aspect-square rounded-[10px] bg-gray-100 animate-pulse" aria-hidden="true" />
                ))}
              </div>
            </div>
          )}
