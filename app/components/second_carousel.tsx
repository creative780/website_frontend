// Front_End/app/components/second_carousel.tsx
'use client';
...
export default function SecondCarousel() {
    ...
    (async () => {
      try {
        ...
        if (carRes.ok) {
          ...
          setCarouselRaw((p) => (p.images.length ? { ...p, images: [] } : p));
        }
      } catch (err: any) {
  ...
  // measure viewport and compute card width (post-paint; guard redundant sets)
  useEffect(() => {
    const measure = () => {
  ...
  const carouselId = useId() || 'second-carousel';
        ...
        >
          {hasImages && !loading ? (
            <div
              ...
              // Stabilize initial paint to avoid jump before cardWidth resolves
